...
urlpatterns = [
    ...
    path(
        "remove-course/<slug:course_slug>/", views.remove_course, name="remove_course"
    ),
    path("edit_profile/", views.edit_profile, name="edit_profile"),
    path("delete-account/", views.delete_account, name="delete_account"),
    path(
        "email/unsubscribe/<uidb64>/", views.unsubscribe_email, name="unsubscribe_email"
    ),
    path("activate/<str:uidb64>/<str:token>/", views.activate, name="activate"),
    path("activation-failed/", views.activation_failed, name="activation_failed"),
    path(
        "resend_activation/<uidb64>/",
        views.resend_activation_email,
        name="resend_activation",
    ),
    path(
        "extend_activation/<uidb64>/",
        views.extend_activation_time,
        name="extend_activation",
    ),
    path(
        "password-reset/",
